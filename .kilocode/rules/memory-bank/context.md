...
| 2026-03-06 | Added Advanced Admin features with mock data + APIs: Data Scope Permission (branch/department/device type), Role Templates with quick user assignment, Permission Diff API/UI, Config Audit logs (before/after + actor), and Security Policy (password/session + terminate all sessions) |
| 2026-03-06 | Refactored Permission UX per workflow: menu label kept as `Phân quyền`, retained 2 horizontal tabs under this section, integrated Data Scope with Select-All at Branch/Department/Device level, filtered Department/Device lists by selected Branch/Department, removed Role Template & Diff UI from AdminTab, and added backend device row-level filtering by user/profile scope (`branchIds`, `departmentIds`, `deviceIds`) in `/api/devices` |
